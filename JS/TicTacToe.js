...
//Disable the stop button since it is not needed until the game starts.
...
function rollForTurn() {
    ...
    // determine and concatenate string showing which player has the roll
    if (pOne > pTwo) {
        first = "Player 1";
...
function startGame() {
    ...
    btnDisabled(btn); // disable the button since the game has started
    var btn = document.getElementById("btnStop");
...
function startEnabled(btn) {
    btn.style.color = "#fff";
    btn.style.border = "2px soldi rgb(0, 153, 0)";
...
function stopGame() {
    hideGameMsg(); // clear the text and hide the message box
    var btn = document.getElementById("btnStart");
    startEnabled(btn); //enable the start button since the game is stopped
    var btn = document.getElementById("btnStop");
...
function hideGameMsg() {
    clearMsg() //clear the text box from the message console
...
// this function return's the currently assigned avatar to each player
function getAvatars() {
...
function determineAvatar() {
    ...
    if (active == "Player 1") { //check which player is active and their corrersponding avatar
        var paintAvatar = p1Avatar;
    } else if (active == "Player 2") {
        var paintAvatar = p2Avatar;
    }
    return paintAvatar; // retunred back the correct avatar
}
...
function avatarPlaced() {
    ...
    check4Tie(); // call this function to inquire if there was a tie game.
}
...
function checkForWinCon() {
    ...
    info = info.split(","); //seaparate the string by commas into an array
    ...
    // call this following array of functions to check for any possible win cons
...
// whenever a win is detected the corresponding funtion will
...
function glowBoard(pos) {
    ...
    for (var i =0;i<squares.length;i++) {
        if (i == index0) {
            ...
            setTimeout(function() {bg1.style.backgroundColor = "#d7f2f7";}, 1100);
        }   else if (i == index1) {
            var bg2 = squares[i];
            setTimeout(function() {bg2.style.backgroundColor = "rgb(66, 244, 256)";}, 100);
            ...
            setTimeout(function() {bg2.style.backgroundColor = "rgb(144, 235, 66)";}, 900);
            setTimeout(function() {bg2.style.backgroundColor = "rgb(244, 273, 66)";}, 1000);
            ...
            setTimeout(function() {bg3.style.backgroundColor = "rgb(297, 244, 66)";}, 800);
            setTimeout(function() {bg3.style.backgroundColor = "rgb(122, 244, 66)";}, 900);
            setTimeout(function() {bg3.style.backgroundColor = "rgb(86, 244, 286)";}, 1000);
...
function squareSound() {
    ...
    sound.play();
    setTimeout(function() {sound.pause();}, 400); // add delay to these to keep the sound short
...
function winSound() {
    ...
    setTimeout(function() {sound.play();}, 500);
    setTimeout(function() {sound.pause();}, 2700); // add delay to these to keep the sound short
...
function blink() {
    ...
    setTimeout(function() {body.style.backgroundColor = "#eD0202";}, 1000);
    setTimeout(function() {body.style.backgroundColor = "#ffffff";}, 1100);
}

// ============================================================================================
//  These functions are the alforithms to find all win conditions
// ============================================================================================
...
function checkWinCon1(info,squareArray) {
    ...
    // avatars and compare all 3 for wincons
    for (var i in info) {
...
function checkWinCon2(info,squareArray) {
    ...
    winner(winDetected,winCon2); // winCon2 is the array of the win combo
}
...
function checkWinCon3(info,squareArray) {
    ...
    // this will trigger (ONLY) if there was a match for index6, index7, and index8
...
function checkWinCon4(info,squareArray) {
    ...
    // this will trigger (ONLY) if there was a match for index0, index3, and index6
...
function checkWinCon5(info,squareArray) {
    ...
    // this will trigger (ONLY) if there was a match for index1, 4, and index7
    if (match1Avatar != undefined && match4Avatar != undefined && match7Avatar != undefined) {
        if (match1Avatar == match7Avatar && match4Avatar == match7Avatar) {
            winDetected = "win"; // this flag will pass when a win has been detected
        }
    }
...
function checkWinCon6(info,squareArray) {
    ...
    // this will trigger (ONLY) if there was a match for index2, index5, and index8
...
function checkWinCon7(info,squareArray) {
    ...
    // this will trigger (ONLY) if there was a match for index0, index4, and 8
...
function checkWinCon8(info,squareArray) {
    ...
    // this will trigger (ONLY) if there was a match for index2, index4, and index6
...
// ============================================================================================
// These block of functions are for each click event in their corresponding square element
// ============================================================================================
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") { // if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined) { // if verdict is empty then the square is unoccupied.
            ...
            checkForWinCon(); // call function to check if the current move completes a winning condition.
            avatarPlaced(square,paintAvatar); // end the current turn and pass the turn to the other player
